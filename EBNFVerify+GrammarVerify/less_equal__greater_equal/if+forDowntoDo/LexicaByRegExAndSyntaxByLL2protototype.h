/*
Њерев?рка прототипу LL(2)-синтаксичного анал?затора (спец?альна структура) та прототипу лексичного анал?затора (регул§рн? вирази) за допомогою коду. Оексеми дл§ синтаксичного анал?затора обробл§ютьс§ лексичним анал?затором, тому синтаксичний анал?затор не анал?зуѓ Ьх повторно (§к показано в ЦљНС).
(пом?стити у файл "LexicaByRegExAndSyntaxByLL2protototype.h")
ФђЉvЉ: при коп?юванн? зважайте, щоб у кожному р§дку п?сл§ символу А\њ не м?стилось жодних ?нших символ?в.
*/
...
#define KEYWORDS_RE       ";|:=|=:|\\+|-|\\*|,|==|!=|\\[|\\]|\\(|\\)|\\{|\\}|NAME|DATA|BODY|END|GET|PUT|IF|ELSE|FOR|DOWNTO|DO|DIV|MOD|<=|>=|NOT|AND|OR|INTEGER16"
...
{LA_IS, { T_FOR_0 }, { "fordownto_cycle",{\
    {LA_IS, {""}, 5, { T_FOR_0, "cycle_counter_init",  T_DOWNTO_0, "cycle_counter_last_value", "cycle_body" }}\
}}},\
{LA_IS, { "ident_terminal", "(", T_NOT_0, "unsigned_value_terminal", T_ADD_0, T_SUB_0, T_IF_0, T_FOR_0, T_INPUT_0, T_OUTPUT_0, T_SEMICOLON_0 }, { "statement__or__block_statements",{\
    {LA_IS, {""}, 1, { "statement" }}\
}}},\
...
{ LA_IS, { "ident_terminal", "(", T_NOT_0, "unsigned_value_terminal", T_ADD_0, T_SUB_0, T_IF_0, T_FOR_0, T_INPUT_0, T_OUTPUT_0, T_SEMICOLON_0 }, { "statement__iteration",{\
    { LA_IS, {""}, 2, { "statement", "statement__iteration" }}\
}}},\
{ LA_NOT, { "ident_terminal", "(", T_NOT_0, "unsigned_value_terminal", T_ADD_0, T_SUB_0, T_IF_0, T_FOR_0, T_INPUT_0, T_OUTPUT_0, T_SEMICOLON_0 }, { "statement__iteration",{\
    { LA_IS, {""}, 0, { "" }}\
}}},\
