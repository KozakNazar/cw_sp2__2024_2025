...
void setDPDA1InstructionByPrecursorId(MarkedRule& multiRule/*, DPDA1Program& dpda1Program*/, DPDA1Instructions& dpda1Instructions, unsigned char precursorId) {
	...
	bool stateIsTwiÒeChanged = false;
	...
	stateIsTwiÒeChanged |= dpda1Instructions[precursorId].tapeAction != NO_SCROLL;
	dpda1Instructions[precursorId].tapeAction = NO_SCROLL;
	/* defaule */stateIsTwiÒeChanged |= dpda1Instructions[precursorId].stackUpdate.stackAction != POP_AND_MULTIPLIPUSH;
	/* defaule */dpda1Instructions[precursorId].stackUpdate.stackAction = POP_AND_MULTIPLIPUSH;
	stateIsTwiÒeChanged |= dpda1Instructions[precursorId].rhsVariantAddonIndexMask != (unsigned char)~0;
...
//#endif
		...
		for (int rhsElementIndex = 0; rhsElementIndex < MAX_RTOKEN_COUNT; ++rhsElementIndex)
			if (rhsElementIndex < multiRule.rule.rhss[rhsVariantIndex].rhs_count) {
				...
				stateIsTwiÒeChanged |= dpda1Instructions[precursorId].stackUpdate.stackAddon[rhsVariantIndex][rhsElementIndex] != newStackElement;
				...
				stateIsTwiÒeChanged |= dpda1Instructions[precursorId].stackUpdate.stackAddon[rhsVariantIndex][rhsElementIndex] != POP_STACK_IN_F_OUT_STATE_ID;
				...
				stateIsTwiÒeChanged |= dpda1Instructions[precursorId].stackUpdate.stackAddon[rhsVariantIndex][rhsElementIndex] != EMPTY_TOKEN_LEXEM_ID;
		...
		//#if !defined(BUILD_AST_BY_DPDA1) || defined(BUILD_P2C_AST_TYPE_BY_DPDA1)
					...
					//char popStackInFoutStateCode = getLexemId((char*)"POP_STACK_IN_F_OUT_STATE");
							...
							// not init or not to dead state
		if (stateIsNotDefault && stateIsTwiÒeChanged) { // ?
			printf("ERROR: no support model\r\n.");
			exit(0);
		}
...
//#define ROW_INDEX firstMarkCodeSelector // firstMarksIndex
					...
					bool stateIsTwiÒeChanged = false;
						...
						stateIsTwiÒeChanged |= dpda1Program[firstMarkCode][stackTopElementCode].tapeAction != NO_SCROLL;
						dpda1Program[firstMarkCode][stackTopElementCode].tapeAction = NO_SCROLL;
						/* defaule */stateIsTwiÒeChanged |= dpda1Program[firstMarkCode][stackTopElementCode].stackUpdate.stackAction != POP_AND_MULTIPLIPUSH;
						/* defaule */dpda1Program[firstMarkCode][stackTopElementCode].stackUpdate.stackAction = POP_AND_MULTIPLIPUSH;
						stateIsTwiÒeChanged |= dpda1Program[firstMarkCode][stackTopElementCode].rhsVariantAddonIndexMask != (unsigned char)~0;
		...
		//#endif
							...
							for (int rhsElementIndex = 0; rhsElementIndex < MAX_RTOKEN_COUNT; ++rhsElementIndex)
								if (rhsElementIndex < multiRule->rule.rhss[rhsVariantIndex].rhs_count) {
									unsigned char newStackElement = getLexemId(multiRule->rule.rhss[rhsVariantIndex].rhs[rhsElementIndex]);
									stateIsTwiÒeChanged |= dpda1Program[firstMarkCode][stackTopElementCode].stackUpdate.stackAddon[rhsVariantIndex][rhsElementIndex] != newStackElement;
									...
									stateIsTwiÒeChanged |= dpda1Program[firstMarkCode][stackTopElementCode].stackUpdate.stackAddon[rhsVariantIndex][rhsElementIndex] != POP_STACK_IN_F_OUT_STATE_ID;
									...
									stateIsTwiÒeChanged |= dpda1Program[firstMarkCode][stackTopElementCode].stackUpdate.stackAddon[rhsVariantIndex][rhsElementIndex] != EMPTY_TOKEN_LEXEM_ID;
					...
					//char popStackInFoutStateCode = getLexemId((char*)"POP_STACK_IN_F_OUT_STATE");
							...
							if (multiRuleIndex > (unsigned char)~0) {
								printf("ERROR: no support model\r\n.");
								exit(0);
							}
...
void print_pda_by_transition_table_to_file(char* fileName, char* tableName/*, int state_counter*/, DPDA1Program& dpda1Program, DPDA1Instructions& dpda1Instructions, PrecursorIds& precursorIds, DPDA1IndexingForSecondElement& dpda1IndexingForSecondElement/*, int dead_state*/, bool useShortTable = true) {
	...
	fprintf(f, "int %sInstructions[LL2_PRECURSOR_COUNT] = {\n", tableName);
	for (unsigned int precursorId = 0; precursorId < LL2_MAX_STATES; ++precursorId) {
	...
	for (int topStackCode = 0; topStackCode < LL2_MAX_STATES; ++topStackCode) {
		...
		if (!useShortTable && topStackCode == EMPTY_TOKEN_LEXEM_ID) {
			sprintf(part_buffer, "  \"\" ");
			//fprintf(f, "  %s ", lexemStr.c_str());
		}
		else if (!useShortTable && topStackCode && getLexemStr(topStackCode, lexemStr)) {
...
// used
// tape scroll
void buildAcceptTapeElement__DPDA1forLL2(Grammar& grammar, DPDA1Program& dpda1Program, PrecursorIds& precursorIds, DPDA1IndexingForSecondElement& dpda1IndexingForSecondElement) {
	...
	unsigned char tapeAndTopStackCode = 0; do {
			// not init or not to dead state
			if (//dpda1Program[ROW_INDEX][columnIndexSelector].tapeAction != -1
				dpda1Program[tapeAndTopStackCode][tapeAndTopStackCode].tapeAction != SCROLL_TO_RIGHT
				||
				dpda1Program[tapeAndTopStackCode][tapeAndTopStackCode].stackUpdate.stackAction != POP_AND_MULTIPLIPUSH
				||
				dpda1Program[tapeAndTopStackCode][tapeAndTopStackCode].rhsVariantAddonIndexMask != 0
				||
				dpda1Program[tapeAndTopStackCode][tapeAndTopStackCode].stackUpdate.stackAddon[0][0] != EMPTY_TOKEN_LEXEM_ID
				&& (
					dpda1Program[tapeAndTopStackCode][tapeAndTopStackCode].stackUpdate.stackAddon[0][0] != DEAD_STATE_ID
					||
					dpda1Program[tapeAndTopStackCode][tapeAndTopStackCode].stackUpdate.stackAddon[0][1] != EMPTY_TOKEN_LEXEM_ID
					)
				) {
				printf("ERROR: no support model\r\n.");
				exit(0);
			...
			// tape
			// dpda1Program[tapeAndTopStackCode][tapeAndTopStackCode].tapeAction = SCROLL_TO_RIGHT; // SHIFT
			...
			for (unsigned int rTokekIndex = 0; rTokekIndex < MAX_RTOKEN_COUNT; ++rTokekIndex) { // not useed for dead state
				dpda1Program[tapeAndTopStackCode][tapeAndTopStackCode].stackUpdate.stackAddon[0/*rhsVariantAddonIndex*/][rTokekIndex] = EMPTY_TOKEN_LEXEM_ID;
			}

#ifndef NO_ACCEPTANCE_BEHAVIOR		
			precursorIds[tapeAndTopStackCode][tapeAndTopStackCode] =
#ifdef DEAD_STATE_ID // regardless of the value DEAD_STATE_ID will be encoded as 0				
				1 +
#endif				
				0				
...
void buildDPDA1forLL2(Grammar& grammar, DPDA1Program& dpda1Program, DPDA1Instructions& dpda1Instructions, PrecursorIds& precursorIds, DPDA1IndexingForSecondElement& dpda1IndexingForSecondElement, REMOVE___OR_NOT) {
	...
	// 123
	//
	buildAcceptTapeElement__DPDA1forLL2(grammar, dpda1Program, precursorIds, dpda1IndexingForSecondElement);
