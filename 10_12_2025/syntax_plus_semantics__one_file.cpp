...
*               file: syntax_plus_semantics__one_file.cpp   *
*                                                  (draft!) *
*************************************************************/
...
int main(int argc, char* argv[]) {
	...
	if (ifBadLexemeInfo.tokenType == UNEXPEXTED_LEXEME_TYPE) {
		...
		if (SUCCESS_STATE != syntaxAnalyze(lexemesInfoTable, &grammar, 0, (char*)AST_OUTPUT_FILE, (char*)&errorMessagesPtrToLastBytePtr, true)
			|| SUCCESS_STATE != semanticsAnalyze(lexemesInfoTable, &grammar, identifierIdsTable, (char*)&errorMessagesPtrToLastBytePtr)) {
				(void)getchar();
				printf("\r\nPress Enter: ");
				(void)getchar();
				system("CLS");
...
//	int returnState = SUCCESS_STATE;
	...
	for (unsigned int index = 0; identifierIdsTable[index][0] != '\0'; ++index) {
		...
		for (unsigned int lexemIndex = 0; lexemesInfoTable[lexemIndex].lexemStr[0] != '\0'; ++lexemIndex) {
			...
			if (!strncmp(lexemesInfoTable[lexemIndex + 1].lexemStr, T_COLON_0, MAX_LEXEM_SIZE)) {
				if (isDeclaredLabel) {
			...
			if (lexemIndex && !strncmp(lexemesInfoTable[lexemIndex - 1].lexemStr, T_GOTO_0, MAX_LEXEM_SIZE)) {
				isLabel = 1;
			}
		}
...
int checkingVariableInitialization(LexemInfo* lexemInfoTable, Grammar* grammar, char(*identifierIdsTable)[MAX_LEXEM_SIZE], char** errorMessagesPtrToLastBytePtr) {
	...
	for (unsigned int index = 0; identifierIdsTable[index][0] != '\0'; ++index) {
		for (unsigned int lexemIndex = lastDataSectionLexemIndex; lexemesInfoTable[lexemIndex].lexemStr[0] != '\0'; ++lexemIndex) {
			...
			if (!strncmp(lexemesInfoTable[lexemIndex - 1].lexemStr, T_LRASSIGN_0, MAX_LEXEM_SIZE)
				||					
				...
				//||					
				!strncmp(lexemesInfoTable[lexemIndex + prevNonOpenParenthesesIndex].lexemStr, T_INPUT_0, MAX_LEXEM_SIZE)
				){						
				break;					
			}
